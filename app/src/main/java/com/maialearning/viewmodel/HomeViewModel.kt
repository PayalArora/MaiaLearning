...
class HomeViewModel(private val catRepository: LoginRepository) : ViewModel(), CoroutineScope {
    ...
    fun compareAll(id: String) {
        showLoading.value = true
        Coroutines.mainWorker {
            val result = withContext(Dispatchers.Main) {
                catRepository.compareAllColleges(id)
            }
            showLoading.value = false
            when (result) {
                is UseCaseResult.Success -> compareAllObserver.value = result.data
                is UseCaseResult.Error -> showError.value = result.exception.message
            }
        }
    }

    fun cancelRound(id: String) {
        showLoading.value = true
        Coroutines.mainWorker {
            val result = withContext(Dispatchers.Main) {
                catRepository.cancelRound(id)
            }
            showLoading.value = false
            when (result) {
