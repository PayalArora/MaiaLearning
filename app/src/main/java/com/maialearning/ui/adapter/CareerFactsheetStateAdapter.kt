...
class CareerFactsheetStateAdapter (fragmentManager: FragmentManager, lifecycle: Lifecycle, size: Int, var response: CareerFactsheetResponse, var sessionDataResponse: SessionDataResponse?) :
    FragmentStateAdapter(fragmentManager, lifecycle) {
    var size = 0
    override fun createFragment(position: Int): Fragment {
        // Hardcoded in this order, you'll want to use lists and make sure the titles match
        if (position == 0) {
            return SummaryNewFragment(response, sessionDataResponse)
        } else if (position == 1) {
