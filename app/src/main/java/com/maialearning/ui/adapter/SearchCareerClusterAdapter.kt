...
class SearchCareerClusterAdapter   (
    val context: Context,
    var arrayListOut: ArrayList<CareerCategoryResponseItem>,
        var click:(s:String, item:CareerCategoryResponseItem)->Unit
) : RecyclerView.Adapter<SearchCareerClusterAdapter.ViewHolder>() {
    /**
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        viewHolder.binding.name.setOnClickListener {
            arrayListOut.get(position).id?.let { it1 -> click(it1,arrayListOut.get(position) ) }
        }
    }
